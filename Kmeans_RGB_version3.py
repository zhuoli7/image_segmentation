...
def downsample(image,down_rate):
	return block_reduce(image, block_size=(down_rate, down_rate, 1), func=np.max)
...
def GMM(image):
	m, n, z = image.shape	
	temp = image.reshape(m*n, z)
	result_GMM = GaussianMixture(n_components = 2).fit(temp)
	means = result_GMM.means_
	covariances = result_GMM.covariances_
	return means, covariances

def get_likelihood(points, centers):
	# creat label with likelihood for segmentation
	dist_a, dist_b = Euclidian_distance(points, centers)
